...
Як читати розміри функцій (size class) — орієнтовна калібровка, підлаштуй числа під темп власної команди:
...
Used at the end of Protocol Step 7 (before Step 8 critic dispatch) and at the end of Step 8 (before finalization commit). The SKILL.md keeps 6-8 inline non-negotiables; the full DoD + anti-patterns + N/A logic + ADR count expectations live here.
...
- [ ] §3 has a `C4Context` Mermaid block with real actors from CONTEXT glossary + real external systems from Explore report (no `<placeholder>` template stubs).
- [ ] §5 has a `C4Container` Mermaid block with real containers / datastores / boundaries (no `<placeholder>`).
- [ ] §6 has ≥1 `sequenceDiagram` Mermaid block. For M+ size: 3-5 sequence diagrams (happy-path + at least one failure-mode flow).
...
- [ ] §11 contains a row for every `save_as_oq`-migrated decision from Step 7 edits-log with both owner AND due filled (no lone owner, no lone due). Missing either → resolution was downgraded to `drop` with a warning.
- [ ] Step 4 Explore subagent ran on brownfield repos (or §3 has `<!-- brownfield: N/A — greenfield repo -->` note).
- [ ] Step 7 edits-log maintained: every `Edit` / `Drop` / `Save as Open Question` resolution appended one entry with `{decision_id, action: edit|drop|save_as_oq, before, after, user_reason}`. `Approve` decisions intentionally absent (baseline).
...
- **§6 Runtime** — never N/A for M+. XS/S may have 1 happy-path flow only (still required); if literally no flow is non-trivial (pure CRUD), document a single happy-path CRUD sequence — never N/A the whole section.
...
- **Returning to a previously-written section** — Step 7e commits each section atomically; Step 7f explicitly forbids reverting to earlier sections. Drift between sections is the critic's job in Step 8. Re-opening §4 after writing §10 means you're not trusting the per-section batch contract.