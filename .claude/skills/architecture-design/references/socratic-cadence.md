...
1. **Одне питання на невизначеність, не на параметр.** Якщо CLAUDE.md уже фіксує defaults (наприклад, конкретна логувальна бібліотека, формат, рівень) — одне питання «Override defaults?», а не три окремі.
