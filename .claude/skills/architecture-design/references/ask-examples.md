...
How Step 7 (Socratic batch loop) and Step 8 (critic resolution) phrase questions and options. The contract from [socratic-loop.md](./socratic-loop.md) and [critic-phase.md](./critic-phase.md) is normative; this file shows the **shape** of the dialogue so options *describe what the skill will do*, not just *what they're labelled*.
...
Options:
  ...
  - label: "Save as Open Question"
    description: "Я приберу це рішення з §4 і додам рядок у §11 Risks: «Open architectural decision: content storage strategy — Open question — Resolve before stage 06 (data-model) — owner: <ти введеш>». Далі я попрошу тебе ввести owner + due одним рядком. Без обох — рішення стає Drop."
...
Options:
  - label: "Lock as ADR (Recommended)"
    description: "Skill creates adr/0001-async-outbox-for-module-integration.md from the AskUserQuestion options + your rationale; adds a row to §9 ADR table; commits sad §4 + adr/0001 together in Step 7e."
  - label: "Inline only"
...
## Critic-finding resolution (Step 8)

```
Question:
  [F1] Strategic-vector drift — §4 caps «async via outbox events» (DEC-§4-modulesIntegration
  Approved at Step 7d), but §6 Critical flow 1 «happy-path goal create» shows synchronous
...
Question:
  ...
  external systems from Explore report (Postgres, notification-service).
  How do you want to resolve it?

Options:
  - label: "Accept regenerate"
    description: "Skill regenerates §3 C4Context block from CONTEXT glossary + PRD §4 US + Explore report. Skill asks once more on the regenerated block (single-iteration cap)."
  - label: "Accept amendment (different wording)"
...
Every `AskUserQuestion` у цьому skill-у формулюється так, щоб **junior-розробник першого року** міг зрозуміти суть рішення, різницю між опціями і наслідки кожної — без помічника поряд. Старий шаблон («1-line WHY», 5-word labels) deprecated для Step 7 + Step 8.
...
3. **Option `description` — 3-5 речень** із чотирьох обов'язкових елементів:
   ...
   - **Наступний механічний крок skill-у**: «я спавню ADR-NNNN з назвою X, додаю рядок у §9 ADR-table, схема фіксується для stage 08 (data-model)»
