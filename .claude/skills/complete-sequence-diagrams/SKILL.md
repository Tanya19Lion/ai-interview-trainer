...
For a single ad-hoc flow, invoke with `--flow <name>` (see "Single-flow mode" below).
...
- `/complete-sequences <slug>` — explicit invocation.
- `/complete-sequences <slug> --flow <name>` — single-flow ad-hoc draw.
...
`/complete-sequences <slug> --flow <name>` — draws one sequence on demand. Skips inventory + coverage; goes straight to step 5 (classify → actors → generate → validate → confirm). Writes inline into SAD §6 under a `### Ad-hoc: <name>` (or `### Endpoint-level: <method path>` for HTTP-verb-aware flows) heading. No coverage report. No separate file is created — the new convention is inline-only.
...
> **User:** "complete sequences for acme-orders"
>
> **Skill behavior:**
> 1. `test -f docs/features/acme-orders/PRD.md && test -f docs/features/acme-orders/sad.md` → OK.
> 2. Inventory: 5 user stories — US-1 createOrder, US-2 listOrders, US-3 biweeklyInventoryReport, US-4 shipOrder, US-5 viewOrderDashboard.
> 3. Coverage table:
>    - US-1 — Covered (heading `### US-1: createOrder` in §6).
...
> 5. **US-3 (async):** classify async (keyword `biweekly`). Actors: Scheduler (NEW — flagged), Worker, order-api, notification-service, DB. Generates sequence with idempotency key check, retry budget Note, DLQ alt. mmdc OK. ADR potential: `consider ADR for retry budget shape`. User confirms.
> 6. **US-5 (sync, perf-critical):** classify sync. Actors from §5: web-app, order-api, DB. Generates sequence with `Note over API, DB: dashboard hits Redis first; cache TTL 60s` folded inline. mmdc OK. User confirms.
> 7. Appends both blocks to sad.md §6 under headings `### US-3: biweeklyInventoryReport` and `### US-5: viewOrderDashboard`.
...
> 10. Commit suggestion: `06: complete sequence coverage for acme-orders`.