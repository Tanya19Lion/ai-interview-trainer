...
| Topic | The subject area a job-seeker chooses once for an entire interview session — one of `react`, `javascript`, `nodejs`, `typescript`, `nextjs`, `css`, `html`, `sql`, `restapi`, `system-design` (`src/models/InterviewSession.ts` `TOPICS`). NOT weak topic. |
