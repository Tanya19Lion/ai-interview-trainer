...
- Contract: [../contracts/openapi.yaml](../contracts/openapi.yaml) `login`/`register`/
  `googleLogin`/`refreshSession` operations
...
- Add `rememberMe?: boolean` to `googleLogin`'s param, `register`'s body type, and
  ...
  `false` when omitted).
- Add a new `refreshSession(): Promise<void>` calling `POST /api/auth/refresh`, matching the
  `refreshSession` operation (no request body, relies on `apiFetch`'s existing `credentials:
  'include'`).
...
- [ ] Manual smoke test against the Prism mock: `refreshSession()` resolves on the mock's 200
      example, rejects (`ApiError`) on its 401 examples.
